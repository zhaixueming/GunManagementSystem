...
void CSystemMangaer::InitVariables()
{
	...
	header <<QString::fromLocal8Bit("枪机或套筒")<<QString::fromLocal8Bit("机枪框号")<<QString::fromLocal8Bit("备份枪管号");
	...
	header << QString::fromLocal8Bit("枪管号") << QString::fromLocal8Bit("枪机或套筒") << QString::fromLocal8Bit("机枪框号") << QString::fromLocal8Bit("备份枪管号");
	header << QString::fromLocal8Bit("  隶属单位  ") << QString::fromLocal8Bit("  管理单位  ") << QString::fromLocal8Bit("  出厂时间  ");
...
{
	// 创建打印机对象
	int row = ui.tableWidget_Single->currentRow();//currentRow为当前item的所在行的行号，从0开始。
	if (row < 0)
	{
...
void CSystemMangaer::QueryInformations()//信息管理界面->“查询”界面中右边的"查询"按钮---》此槽函数
{
	...
	//sql += " and " + QString::fromLocal8Bit("t2.入库时间 BETWEEN \'1995-9-5\' AND \'1995-9-7\'");
	...
	//sql += " and " + QString::fromLocal8Bit("t2.入库时间 BETWEEN \'1995-9-5\' AND \'1995-9-7\' and t2.出库时间 BETWEEN \'1995-12-12\' AND \'1996-9-7\'");
	//sql += " and " + QString::fromLocal8Bit("入库时间 BETWEEN \'1995-09-05\' AND \'1995-09-07\'");
...
{
	int rows = ui.tableWidget_Search->rowCount();//获得tableWidget_Search的行数
	if (rows == 0)
	{
		return;
	}
	...
	//file.open();
	//file.save
	QString filepath = QFileDialog::getSaveFileName(this, ("Save as..."),QString(), tr("EXCEL files (*.xls *.xlsx)"));//获取保存路径
	if (filepath.isEmpty())
	{
		return;
	}
