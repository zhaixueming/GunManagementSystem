...
void CSystemMangaer::InitVariables()
{
	...
	m_bInitAlogSuccess = CAlgoCodeReview::GetInstance()->InitAlgo();
	...
	ui.tableWidget_User->horizontalHeader()->setFont(font);
	ui.tableWidget_User->horizontalHeader()->setHighlightSections(false);
	ui.tableWidget_User->horizontalHeader()->setStyleSheet("QHeaderView::section{color:rgb(255,255,255);background:rgb(41,136,41);}");
	...
	ui.tableWidget_Box->horizontalHeader()->setFont(font);//设置
	...
	header.clear();
	header << QString::fromLocal8Bit("序号") <<QString::fromLocal8Bit("装箱单号") << QString::fromLocal8Bit("装备代码");
	...
	header << QString::fromLocal8Bit("  装备代码  ") << QString::fromLocal8Bit("  装备(部件)名称  ") << QString::fromLocal8Bit("  单装编号  ");
	header << QString::fromLocal8Bit("  隶属单位  ") << QString::fromLocal8Bit("  管理单位  ") << QString::fromLocal8Bit("  出厂时间  ");
	...
	ui.tableWidget_Search->setColumnCount(12);
	header.clear();
	header << QString::fromLocal8Bit(" 退役报废文件号 ") << QString::fromLocal8Bit("退役报废装备处理\n调拨凭证号");
	...
	header << QString::fromLocal8Bit("  检视状态  ") << QString::fromLocal8Bit("  出库状态  ") << QString::fromLocal8Bit("  记录时间  ") << QString::fromLocal8Bit("选择");
	...
	header << QString::fromLocal8Bit("  装箱单号  ") << QString::fromLocal8Bit("  装备代码  ") << QString::fromLocal8Bit("  装备(部件)名称  ");
	header << QString::fromLocal8Bit("  单装编号  ") << QString::fromLocal8Bit("  出厂时间  ") << QString::fromLocal8Bit("  装备时间  ");
	header << QString::fromLocal8Bit("  装箱数量  ") << QString::fromLocal8Bit("  记录时间  ") << QString::fromLocal8Bit("选择");
...
void CSystemMangaer::QueryUserInfo()//账号管理界面的右边“查询”按钮---》此槽函数
{
	...
	QString Number = ui.lineEdit_OfficerNumber->text();//军官证号
	QString sql = "select * from GunManager.dbo.UserTable";
	if (!Name.isEmpty() && !Number.isEmpty())//若有姓名与军官证号
	{
	...
	if (rv)
	{
		if (TableData.size() > 0)
		{
			...
			for (int row = 0; row < TableData.size(); ++row)
			{
				...
				//int XuHao = RowData.at(0).toInt();//获取col位置的数据
...
void CSystemMangaer::EditPackedListInfo()
{
	...
	m_CurDZBianHao.clear();
	m_QSQingKuang.clear();
	m_curMsg.clear();
	m_curMsg.type = 1;
	m_curMsg.index = row+1;
	m_curMsg.PackedNum = DanHao;
...
void CSystemMangaer::DeletePackedListInfo()
{
	...
	if (QMessageBox::question(this, QString::fromLocal8Bit("提示"), QString::fromLocal8Bit("确实要删除吗?"), QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes)
	{
		...
		QString sql = "delete from GunManager.dbo.BoxPackedDetailsTable where " + QString::fromLocal8Bit(" 单装编号 = \'") + BianHao + "\'";
		sql += QString::fromLocal8Bit("and 序号 = \'") + XuHao + "\'";
		QMessageBox::information(this, "check", QString::fromLocal8Bit("看看是啥 ") + XuHao);
...
void CSystemMangaer::EditSinglePacked()
{
	...
	ui.label_DanHao->setText("");
	ui.lineEdit_Ocr->setText("");
	ui.lineEdit_XiuZheng->setText("");
	ui.checkBox_QiangGuan->setEnabled(false);
	...
	m_curMsg.index = row;
	m_curMsg.type = 2;
	m_curMsg.BFWenJianHao = ui.tableWidget_Single->item(row, 0)->text();
	m_curMsg.DBPingZhengHao = ui.tableWidget_Single->item(row, 1)->text();
	m_curMsg.ZBDaiMa = ui.tableWidget_Single->item(row, 2)->text();
	ui.tabWidget->setCurrentIndex(4);
	m_PreviousIndex = 2;
}
...
void CSystemMangaer::QuerySinglePacked()//编码检视界面下，单装入库界面下右边"查询"按钮--》此槽函数
{
...
void CSystemMangaer::PrintSinglePacked()//编码检视界面->"单装入库"界面,左下角“打印单装卡片”按钮--》此槽函数
{
	...
	content.push_back(ui.tableWidget_Single->item(row, 7)->text());//出厂时间
	content.push_back(ui.tableWidget_Single->item(row, 8)->text());//装备时间
	content.push_back(ui.tableWidget_Single->item(row, 5)->text());//隶属单位
	content.push_back(ui.tableWidget_Single->item(row, 6)->text());//管理单位
...
void CSystemMangaer::DoDelivery()//信息管理界面-》“出库”界面的左上“导出Excel”按钮-----》此槽函数
{
	int rows = ui.tableWidget_Delivery->rowCount();
	...
	for (int i = 0; i < rows; ++i)
	{
		...
		if (box != NULL && box->isChecked())
		{
			...
			QString ZBDaiMa = ui.tableWidget_Delivery->item(i, 3)->text();
			QString ZBMingCheng = ui.tableWidget_Delivery->item(i, 4)->text();
			QString DZBianHao = ui.tableWidget_Delivery->item(i, 5)->text();
...
void CSystemMangaer::ReceiveImage(int index, Mat image)//从主界面接收到图像数据
{
	if (index == 1)//相机1
	{
	...
	{
		QImage img = matToQImage(image);//调用matToQImage函数，获取图像数据，行。列，格式等
		ui.label_Image2->SetImage(img);//加锁??????迷糊，以后补充//////“拍照检视”界面，中间两个显示框加上下面的两个按钮
	}
}
