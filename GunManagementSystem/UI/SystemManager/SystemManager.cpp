...
#include "qdatetime.h"
#include "speech.h"
#include "qtxlsxwriter\include\xlsxdocument.h"
#include "qtxlsxwriter\include\xlsxformat.h"
...
void CSystemMangaer::QueryDeliveryInformations()//信息管理界面->“出库”界面中右边的"查询"按钮---》此槽函数
{
	...
	QString sql = QString::fromLocal8Bit("select t1.退役报废文件号,t1.调拨凭证号,t1.装箱单号,t1.装备代码,t1.装备名称,t2.单装编号,t2.枪管号,t2.枪机或套筒号,t2.机枪框号,t2.备份枪管号,t1.出厂时间,t1.装备时间,t1.装箱数量,t2.入库时间");
	sql += " from GunManager.dbo.BoxPackedTable t1 right join GunManager.dbo.BoxPackedDetailsTable t2 on";
	sql += QString::fromLocal8Bit(" t1.装箱单号 = t2.装箱单号 and t1.装备代码 = t2.装备代码 and t1.装备名称 = t2.装备名称");
	...
	sql.clear();
	sql = QString::fromLocal8Bit("select 退役报废文件号,调拨凭证号,\'-\' as 装箱单号,装备代码,装备名称,单装编号,枪管号,枪机或套筒号,机枪框号,备份枪管号,出厂时间,装备时间,\'-\' as 装箱数量,入库时间 from GunManager.dbo.SinglePackedTable");
...
//导出为.xlsx格式，全程登记表(没做出来)
//void CSystemMangaer::ExportGridA9()
//{
//	//获取创建的.xlsx文件
//	//QString fileName = QFileDialog::getSaveFileName(this, tr("Excel file"), qApp->applicationDirPath(), tr("Files (*.csv)"));
//	QString fileName = QFileDialog::getSaveFileName(this, ("Save as..."), qApp->applicationDirPath(), tr("EXCEL files (*.xls *.xlsx)"));
//	if (fileName.isEmpty())
//	{
//		return;
//	}
...
//
//
//	qDebug() << "Export excel:" << fileName.toStdString().c_str();
//
...
//	//			int xlsxRow = i + 5;
//	//			rowStr = ui.tableWidget_Delivery->item(i, j)->text();
//	//			outxlsx.write(xlsxRow, j, rowStr);
//	//			//out << rowStr << ",";// 写入文件
//	//		}
//	//		//out << "\n" << ",";
//	//		//out << "\n";
...
void CSystemMangaer::SaveRecognizeResult()
{
	...
	QString message_QiangJiHao = QString::fromLocal8Bit("枪机/套筒号:") + m_QiangJiHao + "\n";
	...
	if (mymsgbox->clickedButton() == okbtn)//点击了OK按钮
	{
		...
		if (m_curMsg.type == 1)//箱装
		{
			...
			sql += QString::fromLocal8Bit(",枪机或套筒号 = \'") + m_QiangJiHao + "\'";
