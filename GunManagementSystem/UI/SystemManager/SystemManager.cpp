...
void CSystemMangaer::InitVariables()
{
	...
	header.clear();
	header <<QString::fromLocal8Bit("装箱单号") << QString::fromLocal8Bit("装备代码") << QString::fromLocal8Bit("装备(部件)名称") << QString::fromLocal8Bit("枪支类型");
	...
	ui.tableWidget_Single->setColumnWidth(12, 110);//枪支类型
...
void CSystemMangaer::EditPackedListInfo()
{
	...
	QString QiangZhiType = ui.tableWidget_PackedInfo->item(row, 3)->text();//获取箱装列表的第4列为箱装单号显示
